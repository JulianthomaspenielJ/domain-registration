...
class CLIENTMODEL extends APIRESPONSE
{
    ...
    public function getClientDetails($data, $loginData)
    {
        try {
            ...
            $sql = "SELECT id,client_name,email_id, mobile_no,address,created_by, created_date, updated_date FROM tbl_clients   WHERE status = 1 and created_by = " . $loginData['user_id'] . " ORDER BY id DESC LIMIT " . $start_index . "," . $end_index . "";
            ...
            if ($row_cnt > 0) {
                while ($product_row = $result->fetch_assoc()) {
                    ...
                    $sql2 = "SELECT id,name,email_id,mobile_no,client_id ,designation_id, created_date, updated_date FROM tbl_contact_person   WHERE status = 1 and created_by = " . $loginData['user_id'] . " and client_id=" . $product_row['id'] . " ORDER BY id DESC LIMIT " . $start_index . "," . $end_index . "";
    ...
    public function getClient($data, $loginData)
    {
        try {
            ...
            $clientData = array();
            $db = $this->dbConnect();
            $sql = "SELECT id,client_name,email_id, mobile_no,address,created_by, created_date, updated_date FROM tbl_clients	WHERE status = 1 and created_by = " . $loginData['user_id'] . " and id =$id";
            $result = $db->query($sql);
            $row_cnt = mysqli_num_rows($result);
            $clientData = array();
            $contactData = array();
            if ($row_cnt > 0) {
                ...
                $sql2 = "SELECT id,name,email_id,mobile_no,client_id ,designation_id, created_date, updated_date FROM tbl_contact_person   WHERE status = 1 and created_by = " . $loginData['user_id'] . " and client_id=" . $product_row['id'];
                $result2 = $db->query($sql2);
                while ($product_row2 = $result2->fetch_assoc()) {
    ...
    private function createClient($data, $loginData)
    {
        ...
        try {
            ...
            $insertQuery = "INSERT INTO tbl_clients (client_name,email_id, mobile_no,address,status, created_by, created_date) VALUES ('" . $data['client_name'] . "','" . $data['email_id'] . "','" . $data['mobile_no'] . "','" . $data['address'] . "','" . '1' . "','" . $loginData['user_id'] . "','$dateNow')";
    ...
    private function updateClient($data, $loginData)
    {
        ...
        try {
            ...
            $this->validateInputDetails($validationData);
            $dateNow = date("Y-m-d H:i:s");
            $updateQuery = "UPDATE tbl_clients SET client_name = '" . $data['client_name'] . "', email_id = '" . $data['email_id'] . "',mobile_no = '" . $data['mobile_no'] . "',updated_by = '" . $loginData['user_id'] . "',updated_date = '$dateNow' WHERE id = " . $data['id'] . "";
